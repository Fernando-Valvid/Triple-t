...
if (isset($_REQUEST['action'])) {
    switch ($_REQUEST['action']) {
        case 'actualizar':
            ...
            $des->__SET('nombre_edo', $_REQUEST['nombre_edo']);
            $des->__SET('img_edo', $_REQUEST['img_edo']);


            $model->Actualizar($des);
            header('Location: estados.php#pricing');
            break;
        ...
        case 'registrar':
            ...
            if(isset($_FILES["foto"])){
                ...
                if($tipo != 'image/jpg' && $tipo != 'image/JPG' && $tipo != 'image/jpeg' && $tipo != 'image/png' && $tipo != 'image/gif' && $tipo != 'image/JPEG'){
                    ...
                    $img_edo="../../fotosedo/".$nombre;
                }
            }
            $des->__SET('img_edo', $img_edo);
            $model->Registrar($des);
	...
	window.location.href='estados.php'; 
        ...
        case 'eliminar':
            $model->Eliminar($_REQUEST['id_estado']);
           print "<script language='JavaScript'>
    	alert('Datos Eliminados Exitosamente' );  
	window.location.href='estados.php'; 
	</script>";;
        ...
        case 'editar':
            $cdes = $model->Obtener($_REQUEST['id_estado']);
            break;
    }
}
?>
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title"> Gestiona Los Estados</h4>
                   <form id="login" action="?action=<?php echo $des->id_estado > 0 ? 'actualizar' : 'registrar'; ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="id_estado" value="<?php echo $des->__GET('id_estado'); ?>" />
                                        <div class="form-group">
                                            <label>Nombre del Estado</label>
                                            <input type="text" name="nombre_edo" required="" value="<?php echo $des->__GET('nombre_edo'); ?>"  class="form-control" placeholder="Ingresa la Nombre del Estado">
                                        </div>
                                        <div class="form-group">
                                            <label>Imagen del Estado</label>
                      ...
                      <thead class=" text-primary">
                                    ...
                                    <tbody>
                                        <tr class="odd gradeX">
                                            ...
                                            <td><?php echo ' <img src="'.$r->__GET('img_edo').'" width=300px height=auto> '; ?></td>
                                           
                                            <td class="center"><a href="?action=eliminar&id_estado=<?php echo $r->id_estado; ?>"><button id="" type="button" class="btn btn btn-danger">Eliminar</button></a></td>
