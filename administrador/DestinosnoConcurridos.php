...
require_once '../Control/destinonoconcurrido/destinono_model.php';
require_once '../Control/destinonoconcurrido/entidad_destinono.php';


$des = new DestinoNo();
$model = new DestinoNoModel();

if (isset($_REQUEST['action'])) {
    switch ($_REQUEST['action']) {
        case 'actualizar':
            $des->__SET('id_nconcurrido', $_REQUEST['id_nconcurrido']);
            $des->__SET('id_destino', $_REQUEST['id_destino']);

            $model->Actualizar($des);
            header('Location: DestinosnoConcurridos.php#pricing');
            break;
	...
	window.location.href='DestinosnoConcurridos.php'; 
	</script>";

            break;

        case 'eliminar':
            $model->Eliminar($_REQUEST['id_nconcurrido']);
	...
	window.location.href='DestinosnoConcurridos.php'; 
        ...
        case 'editar':
            $cdes = $model->Obtener($_REQUEST['id_nconcurrido']);
            break;
    }
}
?>
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title"> Agrega Los Destinos No Concurridos</h4>
                  <form id="login" action="?action=<?php echo $des->id_nconcurrido > 0 ? 'actualizar' : 'registrar'; ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="id_nconcurrido" value="<?php echo $des->__GET('id_nconcurrido'); ?>" />
