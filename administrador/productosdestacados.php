...
require_once '../Control/productosdestacados/productodes_model.php';
require_once '../Control/productosdestacados/entidad_productodes.php';


$des = new Destacado();
$model = new DestacadoModel();

if (isset($_REQUEST['action'])) {
    switch ($_REQUEST['action']) {
        case 'actualizar':
            $des->__SET('id_destacados', $_REQUEST['id_destacados']);
            $des->__SET('id_pro', $_REQUEST['id_pro']);

            $model->Actualizar($des);
            header('Location: productosdestacados.php#pricing');
            break;
	...
	window.location.href='productosdestacados.php'; 
	</script>";

            break;

        case 'eliminar':
            $model->Eliminar($_REQUEST['id_destacados']);
	...
	window.location.href='productosdestacados.php'; 
        ...
        case 'editar':
            $des = $model->Obtener($_REQUEST['id_destacados']);
            break;
    }
}
?>
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                <h4 class="card-title"> Agrega Los Productos Populares</h4>
                  <form id="login" action="?action=<?php echo $des->id_destacado > 0 ? 'actualizar' : 'registrar'; ?>" method="post" enctype="multipart/form-data">
                                <input type="hidden" name="id_destacado" value="<?php echo $des->__GET('id_destacado'); ?>" />
                                        <div class="form-group">
                                            <label>Nombre del Destino</label>
                                            <select class="form-control" name="id_pro">
                                    <option class="form-control" value="0">Seleccione un Producto</option>
                                    <?php
                                    $mysqli = new mysqli('localhost', 'u205223607_ttt', 'Gp0Empr3sarialV4lv1d#101#', 'u205223607_ttt');

                                    $query = $mysqli->query("SELECT * FROM productos");
                                    while ($des = mysqli_fetch_array($query)) {
                                        echo '<option  value=" '.$des['id_pro'].' "> '.$des['nombre_pro'].' </option>';
                                    }
                                    ?> 
                                </select>
                      ...
                      <thead class=" text-primary">
                                    ...
                                    <tbody>
                                        <tr class="odd gradeX">
                                            ...
                                            <td><?php echo ' <img src="'.$r->__GET('img1_pro').'" width=300px height=auto> '; ?></td>
                                           
                                            <td class="center"><a href="?action=eliminar&id_destacados=<?php echo $r->id_destacados; ?>"><button id="" type="button" class="btn btn btn-danger">Eliminar</button></a></td>
